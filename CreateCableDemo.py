import carb
import math
from pxr import UsdLux, UsdGeom, Sdf, Gf, UsdPhysics, UsdShade, PhysxSchema, PhysicsSchemaTools, Vt
...
class RigidBodyRopesDemo(demo.Base):
    ...
    def setup_physics_scene(self, stage, primPathAsString = True, metersPerUnit = 1, gravityMod = 1.0, upAxis = UsdGeom.Tokens.z):
        try:
            currentDemoParams = self.demoParams
    ...
    def create(self, stage, num_ropes, rope_length):
        ...
        self._stage = stage
        self._defaultPrimPath, scene = self.setup_physics_scene(stage, primPathAsString=False)
        # room = demo.get_demo_room(self, stage, zoom=1, hasTable=True, hasWalls=False) # self, stage, zoom=1, hasTable=False, hasWalls=False, onlyUsePlane=True

        # payload config
        self._payloadSize = 0.07
        self._paylaodDensity = 1
        self._payloadColor = [0.56, 0.57, 0.58]
        self._initHangHeight = rope_length + 12.0

        # Rope configuration:
        self.capsuleDensity = 0.1
        self._linkHalfLength = 0.02
        self._linkRadius = 0.01      # radius = 1
        # Capsule total length = height(2) + 2*radius(1) = 2 + 2 = 4 units
        self._ropeLength = rope_length
        self._numRopes = num_ropes
        self._ropeSpacing = 0.150
        self._ropeColor = demo.get_primary_color()
        self._coneAngleLimit = 160
        self._rope_damping = 0.0   # rotation damping
        self._rope_stiffness = 0.0 # rotation stiffness
        self._slideLimit = 10
        ...
        # Table Config
        self._tableHeight = self._initHangHeight - 10 + self._linkRadius / 2
        self._tableSurfaceDim = Gf.Vec2f(2.0, 1.0)
        self._tableColor = Gf.Vec3f(168.0, 142.0, 119.0) / 255.0

        # Physics material:
        self._contactOffset = 0.1
        ...
        # self.create_plane(self, "/World", "GroundPlane")
    ...
    def _createVerticalRopes(self):
        linkLength = self._linkHalfLength * 2
        numLinks = int(self._ropeLength / linkLength)
    ...
    def orient_pos(self, vec):
        ...
        for i in range(3):
            ...
            if curOrientation == 2:
                newVec[i] += 0 # self._floorOffset # shift everything downwards so that the table is at the origin
            newVec[i] *= 1 #s elf._scaleFactor
        return newVec
    
    def orient_dim(self, vec):
        newVec = Gf.Vec3f(0.0)
        for i in range(3):
            newVec[i] = vec[self._orientation[i]] * 1 # self._scaleFactor
        return newVec
    ...
    def create_table(self):
        ...
        # tableTopPath = "/World/table/tableTopActor"
        # UsdPhysics.RigidBodyAPI.Apply(tableTopPath)
    ...
    def createTableRopeJoint(self):
        tableTopPrimPath = "/World/table/tableTopActor"
        for ropeIndex in range(self._numRopes):
            # Path of the rope’s Scope and the point instancer
            ropeScopePath = f"/World/Rope{ropeIndex}"
            ropeInstancerPath = ropeScopePath + "/rigidBodyInstancer"
            fixJointPath = ropeScopePath + "/tableAttachFixJoint"
            fixJoint = UsdPhysics.Joint.Define(self._stage, fixJointPath)
            d6Prim = fixJoint.GetPrim()
            for axis in ["transX", "transY", "transZ", "rotX", "rotY", "rotZ"]:
                limitAPI = UsdPhysics.LimitAPI.Apply(d6Prim, axis)
                limitAPI.CreateLowAttr(0.0)
                limitAPI.CreateHighAttr(0.0)
            fixJoint.GetBody0Rel().SetTargets([tableTopPrimPath])
            fixJoint.GetBody1Rel().SetTargets([ropeInstancerPath])
            d6Prim.CreateAttribute("physics:body1:indices", Sdf.ValueTypeNames.IntArray).Set([0])
            d6Prim.CreateAttribute("physics:localPos0", Sdf.ValueTypeNames.Float3).Set(Gf.Vec3f(0,0,0))
            d6Prim.CreateAttribute("physics:localPos1", Sdf.ValueTypeNames.Float3).Set(Gf.Vec3f(0,0,0.02))
            d6Prim.CreateAttribute("physics:localRot0", Sdf.ValueTypeNames.Quatf).Set(Gf.Quatf(1.0))
            d6Prim.CreateAttribute("physics:localRot1", Sdf.ValueTypeNames.Quatf).Set(Gf.Quatf(1.0))
            return
...
RR = RigidBodyRopesDemo()
RR.create(stage, 1, 0.4)
RR.createTableRopeJoint()
