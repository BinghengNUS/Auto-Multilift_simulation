...
private:
    ...
    // Record for vel_rel
    double last_elongation = 0.0;
    double stepsize = 0.001;

    // Mannual Tune Params
    double segment_length = 0.1;
    double cable_stiffness = 5000;
    ...
    void calculateDamping()
    {
        ...
        geometry_msgs::Point unit_vector_cable = calculateUnitVector(end_pose, start_pose);
        ...
        {   // Calculate damping when cable is taut\stretched
            ...
            double vel_rel = std::abs(elongation - last_elongation) / stepsize;
            // 记录当前elongation为上一时刻，为下一次计算准备
            last_elongation = elongation;

            double damping = std::abs((projected_force - spring_force) / vel_rel);
            // double damping = std::abs((projected_G - spring_force) / vel_rel);
        ...
        else
        {
            ...
            double damping = std::abs((projected_G - 0.0) / vel_rel);
