...
public:
    DistanceCalculator()
    {
        ...
        file.open("/home/carlson/ros/multi_cable/cable_elongation.csv");
        if (!file.is_open()) {
            ROS_ERROR("Failed to open file for writing");
        } else {
            ROS_INFO("CSV file opened successfully");
        }

        file << "Time,Elongation\n"; // Write CSV header
    }
...
private:
    ...
    std::ofstream file; // File stream for CSV output
    ...
    void calculateStiffness()
    {
        ...
        // Get the current ROS simulation time
        ros::Time current_time = ros::Time::now();

        // Write the elongation and time to the CSV file
        if (file.is_open())
        {
            file << current_time.toSec() << "," << elongation << "\n";
        }
