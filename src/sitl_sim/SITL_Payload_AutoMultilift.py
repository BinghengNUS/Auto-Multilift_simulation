...
class RigidBodyRopes(demo.Base):
    ...
    def create(self, stage, num_ropes=6, rope_length=1.0, ropd_stiffness=1e4, rope_damping=1e2, 
               payload_mass=3.0, payload_radius=0.05, payload_height=0.05,
    ...
    def _createPayload(self):
        ...
        # Inertia tensor
        inertia_tensor = 0.5 * np.array([[2, 2, 2.5]]).T  # Jl 
        ...
        # Principal axes
        massAPI.CreatePrincipalAxesAttr().Set(Gf.Quatf(1.0, Gf.Vec3f(0.0, 0.0, 0.0)))
...
class PayloadStatePublisher(Node):
    def __init__(self, pg_app, payload_path, dt_pub=2e-2):
        super().__init__('payload_state_publisher')
        ...
        self.timer = self.create_timer(dt_pub, self.publish_payload_state)
...
def main(args=None):
    ...
    node = PayloadStatePublisher(pg_app, payload_path, dt_pub)
