...
tmux new -d -s "$SESSION" -n main \
  "bash -lc '$ENV && watch -n 0.5 nvidia-smi'"
...
tmux split-window -v -t "$SESSION":0.0 \
  "bash -lc '$ENV && $SIM $DIR/src/sitl_sim/sitl_sim/SITL_AutoMultilift.py'"

# wait until PX4 status appears
$ENV
until ros2 topic echo /px4_0/fmu/out/vehicle_status_v1 --once >/dev/null 2>&1; do sleep 2; done

# pane-3
tmux split-window -v -t "$SESSION":0.1 \
  "bash -lc '$ENV && ros2 launch px4_offboard multilift_mpc_srv.launch.py'"
