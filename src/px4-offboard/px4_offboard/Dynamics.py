...
class multilifting:
    ...
    def minisnap_quadrotor_fig8(self,Coeffx,Coeffy,Coeffz,time,angle_t,index):
        # relative position of the i-th quadrotor in the desired {Bl_d} which is parallel to {I}
        pil   = np.array([[(self.rl+self.cl0*math.sin(angle_t))*math.cos(index*self.alpha),
                        (self.rl+self.cl0*math.sin(angle_t))*math.sin(index*self.alpha),
                        self.cl0*math.cos(angle_t)]]).T 
