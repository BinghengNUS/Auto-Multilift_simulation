    ...
    def compute_drone_trajectories(self):
        ...
        for drone_id in range(self.num_drones):
            ...
            for t in range(self.num_timesteps):
                ...
                # Cable direction and angular velocity in ENU frame
                q_cable_enu = self.cable_dir[drone_id, t]
                omega_cable_enu = self.cable_omega[drone_id, t]

                # Convert to NED frame
                x_l = self.T_enu2ned @ x_l_enu
                v_l = self.T_enu2ned @ v_l_enu
                q_cable = self.T_enu2ned @ q_cable_enu
                omega_cable = self.T_enu2ned @ omega_cable_enu

                # Payload rotation matrix: body -> NED inertial
                R_l_enu = self.quat_to_rotation_matrix(q_l_enu)
                R_l = self.T_enu2ned @ R_l_enu @ self.T_enu2ned  # keep payload orientation in NED

                # Compute drone position
                x_drone = x_l + R_l @ self.rho[drone_id] + self.cable_length * q_cable

                # Cable direction derivative from angular rate: q_dot = omega x q
                q_cable_dot = np.cross(omega_cable, q_cable)

                # R_l_dot from body rates
                omega_l = self.T_enu2ned @ omega_l_enu
                omega_l_hat = self.hat(omega_l)
                R_l_dot = R_l @ omega_l_hat

                # Drone velocity: derivative of x_l + R_l*rho + L*q_cable
                v_drone = v_l + R_l_dot @ self.rho[drone_id] + self.cable_length * q_cable_dot
