...
class QuadNode(Node):
    ...
    def __init__(self):
        super().__init__(f'AutoMultilift_Quadrotor_MPCCtrl')
    ...
    def Qnode_state_machine(self):
        ...
        Main state machine to handle drone offboard states:
          ...
          ST_TRAJECTORY -> Follow the signals from the SyncNode, check REC_TEMP[i] to start & stop QMPC
        ...
        # 3) ST_TAKEOFF: takeoff by Geom Ctrl; switch to MPC trajectory when received MPC_TRAJ=True
            ...
            if (self.MPC_TRAJ): # LNode set MPC_TRAJ = True, start the distributed MPC
                ...
                # NOTE Qnode continuously check the REC_TEMP[i] from LNode to start QMPC, set it faster than LNode
                self.qmpc_forward_timer = self.create_timer(
                    0.001, 
                    self.qmpc_forward_main,
                    callback_group=self.control_logic_callback_group)
                self.Qnode_state = self.ST_TRAJECTORY

        # 4) ST_TRAJECTORY: follow the signals from the SyncNode, check REC_TEMP[i] to start & stop QMPC
        elif self.Qnode_state == self.ST_TRAJECTORY:
            if not self.MPC_TRAJ:
                self.get_logger().info("----- Stopping the distributed MPC -----")
                self.qmpc_forward_timer.cancel()
        ...
        # 5) ST_DONE: received MPC_TRAJ=False, maintains or ends operation
            ...
            # NOTE leave qmpc_ctrl_timer to publish command and maintain the last state
    ...
    # --- QMPC Forward --- 
    def qmpc_forward_main(self):
        # LNode set MPC_TRAJ = True, and currently do not receive the temp_traj, REC_TEMP_i = False
        if (self.MPC_TRAJ and not self.REC_TEMP_i):
            if not self.solved_flag:
                ...
                # 1. Initialize the SAME reference trajectory using same time_traj & angle_t
                # Ref_xq, Ref_uq, Ref_xl, Ref_ul, Ref0_xq, Ref0_l = self.Reference_for_MPC(self.time_traj, self.angle_t)
                ...
                # 3. Solve the QMPC, get the temp_traj and max_viol_i, update into self. and publish to LNode
            ...
            # Continuously publish the temp_traj to LNode, until REC_TEMP_i = True
            self.qmpc_return_temp() 
        
        if self.MPC_TRAJ and self.REC_TEMP_i:
            if not self.confirmed_flag:
                confirmtime = (TM.time() - self.start_time)*1000
                self.get_logger().info(f"drone_idx: {self.drone_idx}, ---confirm_time: {confirmtime:.2f} ms---")
    ...
    def QuadrotorMPC(self,
        ...
        # NOTE Finish current QMPC, update the xi_temp and ui_temp, publish to LNode
        self.xi_temp     = xi_opt
        self.ui_temp     = ui_opt
        ...
        # while not self.REC_TEMP_i: # BUG Whole process is blocked
            # NOTE return the temp_traj to LNode
            # self.qmpc_return_temp() 
            # TM.sleep(0.001) # 100 Hz return QMPC results
    ...
    def _initialize_MPC_states(self):
        ...
        self.REC_TEMP_i = True # Mark as start & end flag of the ith QMPC
        ...
        # NOTE For QMPC's real control inputs, update it when msg.time_traj > self.time_traj
    ...
    def _initialize_publishers(self):
        ...
        self.publisher_QMPC_temp = self.create_publisher(
            QuadReturnMPC,
            f'/quad_{self.drone_idx}/QMPC_temp',
            self.qos_profile
        ) # pub after solved QMPC
    ...
    def _initialize_subscribers(self):
        ...
        # Global Subscriber to get opt_system to solve MPC, and bool state flags
        self.create_subscription(
                BroadcastMPC,
                '/broadcast_mpc',
                self.sync_broadcast_callback,
                self.qos_profile,
                callback_group=self.state_update_callback_group
        )
    ...
    def generate_takeoff_trajectory(self):
        ...
        # z_target = - self.altitude + self.ref_translation[2] # Target altitude in NED local frame
        z_target = - self.altitude # Target altitude in NED local frame
        # self.get_logger().info(f"dt_s: {dt_s}, current_z: {current_z}, z_target: {z_target}")
        ...
        # pos_xy_offset related to the payload's init_position, cal in ENU. Should transform to NED.
        # x_offset_toLoad = self.x_init - self.x_init_load
        # y_offset_toLoad = self.y_init - self.y_init_load
        # x_final = self.x_init_load + x_offset_toLoad / 3.0
        # y_final = self.y_init_load + y_offset_toLoad / 3.0
        # NOTE x_init, y_init; x_final, y_final are in the ENU world frame. xd is in the NED local frame
        # self.get_logger().info(f"x_init: {self.x_init}, y_init: {self.y_init}, x_final: {self.x_final}, y_final: {self.y_final}")
        if dt_s < self.takeoff_time:
            alpha = dt_s / self.takeoff_time
    ...
    # --- Publisher Functions ---
    def qmpc_return_temp(self):
        """Publish the temporary trajectory to the LNode."""
        msg = QuadReturnMPC()
        msg.idx = self.drone_idx
        msg.xi_temp = self.xi_temp.astype(np.float32).flatten().tolist()
        msg.ui_temp = self.ui_temp.astype(np.float32).flatten().tolist()
    ...
    def qmpc_publish_position(self):
        ...
        # if (self.drone_idx == 0):
        #     self.get_logger().info(f"position in world frame: {d_position_enu_world.tolist()}, "
        #                            f"position in local frame: {d_position_ned_local.tolist()}")
    ...
    def recover_traj_list(self, flat_data: list, n_traj: int, traj_shape: tuple) -> list:
        ...
        return traj_list
    def sync_broadcast_callback(self, msg: BroadcastMPC):
        """Callback function for the broadcast message from LNode. --> QMPC, update the real control."""
        self.MPC_TRAJ = msg.mpc_traj
        rec_temp = np.array(msg.rec_temp).reshape((self.nq, 1))
        if (rec_temp[self.drone_idx, 0] == False and self.REC_TEMP_i == True):
            self.solved_flag = False
            self.confirmed_flag = False

        self.REC_TEMP_i = rec_temp[self.drone_idx, 0] # scalar, rec_temp[self.drone_idx] - 1d array
        if (self.REC_TEMP_i == False or self.time_traj == 0): # NOTE Update local QMPC variables
            # self.get_logger().info(f"xq_traj: {msg.xq_traj}")
            self.xq_traj = self.recover_traj_list(msg.xq_traj, self.nq, (self.horizon + 1, self.nxi))
            self.uq_traj = self.recover_traj_list(msg.uq_traj, self.nq, (self.horizon, self.nui))
            self.xl_traj = np.array(msg.xl_traj, dtype=np.float32).reshape((self.horizon + 1, self.nxl))
    ...
    def vehicle_odometry_callback(self, msg: VehicleOdometry):
        ...
        enu_velocity_world = enu_velocity_local  # No need to change, because frame has no rotation

        self.x = ned_position
        # self.last_z = - self.x[2] + self.ref_translation[2] # z in ENU world frame
        self.v = ned_velocity
        ...
        q_ned_to_enu = np.array([0.7071, 0, 0, -0.7071])  # [w, x, y, z]
        ...
        # q0, q1, q2, q3 = q_frd_to_enu
        q0, q1, q2, q3 = msg.q
        # self.qi = np.array([[q0], [q1], [q2], [q3]])
        # self.qi = q_frd_to_ned.reshape((4, 1))
