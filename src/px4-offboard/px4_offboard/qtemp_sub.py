...
class QPResultSubscriber(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(QuadReturnMPC, 'qmpc_result', self.callback, 10)
    ...
    def callback(self, msg):
        quad_idx = msg.idx
        xi_temp = np.array(msg.xi_temp, dtype=np.float32).reshape((self.h+1, self.nxi))
        ui_temp = np.array(msg.ui_temp, dtype=np.float32).reshape((self.h, self.nui))
