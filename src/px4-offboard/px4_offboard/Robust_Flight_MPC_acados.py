...
class MPC:
    ...
    def MPCsolverQuadrotorInit_ros2_acados(self, index, gazebo_sim=False):
        """
        This function is to define the optimal problem using acados, init 1 for each quadrotor node
        """
        # predict horizon in seconds
        T = self.N * self.dt
        ...
        acados_source_path = os.environ['ACADOS_SOURCE_DIR']
        sys.path.insert(0,acados_source_path)
        OCP_q.acados_include_path = acados_source_path + '/include'
        OCP_q.acados_lib_path = acados_source_path + '/lib'

        ##----------mapping CasADi to ACADOS----------##
        Model_q.name="ACADOS_model_q_"+str(index)
    
        # parameters
        Pi = SX.sym('P_'+str(index),(self.n_xi # state reference at one step
                    +self.n_ui # control reference at one step
                    +self.n_pi # hyperparameters
                    +(self.nq-1)*2 # all the quadrotor's x and y states, excluding the i-th quadrotor's state
                    +self.n_xl # the load state at one step
        ...
        # system state and control varialbes
        Model_q.x    = self.xi
        xi_dot      = SX.sym('x_dot_'+str(index),self.n_xi)
        Model_q.xdot = xi_dot
        Model_q.u    = self.ui

        OCP_q.model  = Model_q
        OCP_q.dims.N = self.N # number of nodes
        OCP_q.solver_options.tf = T # horizon length T (unit: second)
        OCP_q.dims.np = self.n_xi + self.n_ui + self.n_pi + (self.nq-1)*2 + self.n_xl + 1 + 1 # number of parameters for the solver
        OCP_q.parameter_values = np.zeros(self.n_xi + self.n_ui + self.n_pi + (self.nq-1)*2 + self.n_xl + 1 + 1) 

        Ref_xi = OCP_q.model.p[0:self.n_xi]
        Ref_ui = OCP_q.model.p[self.n_xi:(self.n_xi+self.n_ui)]
        Para_i = OCP_q.model.p[(self.n_xi+self.n_ui):(self.n_xi+self.n_ui+self.n_pi)]
        xq_i   = OCP_q.model.p[(self.n_xi+self.n_ui+self.n_pi):(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2)]
        xl     = OCP_q.model.p[(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2):(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2+self.n_xl)]
        ti     = OCP_q.model.p[(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2+self.n_xl):(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2+self.n_xl+1)]
        indexi = OCP_q.model.p[(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2+self.n_xl+1):(self.n_xi+self.n_ui+self.n_pi+(self.nq-1)*2+self.n_xl+2)]

        # explicit model
        Model_q.f_expl_expr = self.f_i(xi0=OCP_q.model.x,ui0=OCP_q.model.u,xl0=xl,ti0=ti,i0=indexi)['fif']

        # implicit model
    
        Model_q.f_impl_expr = OCP_q.model.xdot - self.f_i(xi0=OCP_q.model.x,ui0=OCP_q.model.u,xl0=xl,ti0=ti,i0=indexi)['fif']
        ...
        #------------external cost------------#
        OCP_q.cost.cost_type   = 'EXTERNAL'
        OCP_q.cost.cost_type_e = 'EXTERNAL'
        Xq_i = SX.sym('Xq_'+str(index),2,(self.nq-1))
        for k in range(self.nq-1):
            xq_ik = xq_i[k*2:(k+1)*2]
            Xq_i[:,k] = xq_ik
    
        OCP_q.model.cost_expr_ext_cost = self.Ji_kfn(xi0=OCP_q.model.x,ui0=OCP_q.model.u,refxi0=Ref_xi,refui0=Ref_ui,parai0=Para_i)['Ji_kf'] \
            + self.hc_kfn(xi0=OCP_q.model.x,xl0=xl,i0=indexi)['hc_kf'] + self.gq_kfn(xi0=OCP_q.model.x,xqi0=Xq_i)['gq_kf']
        OCP_q.model.cost_expr_ext_cost_e = self.Ji_Nfn(xi0=OCP_q.model.x,refxi0=Ref_xi,parai0=Para_i)['Ji_Nf'] \
            + self.hc_kfn(xi0=OCP_q.model.x,xl0=xl,i0=indexi)['hc_kf'] + self.gq_kfn(xi0=OCP_q.model.x,xqi0=Xq_i)['gq_kf']
        ...
        x_init   = np.zeros(self.n_xi)
        x_init[6]= 1
        OCP_q.constraints.x0 = x_init

        OCP_q.constraints.lbx = np.array(self.xi_lb)
        OCP_q.constraints.ubx = np.array(self.xi_ub)
        OCP_q.constraints.idxbx = np.array([i for i in range(self.n_xi)])

        ##-------control constraints---------##
        OCP_q.constraints.lbu = np.array(self.ui_lb)
        OCP_q.constraints.ubu = np.array(self.ui_ub)
        ...
        OCP_q.code_export_directory = model_folder_path
        # compile acados ocp
        json_file_i = os.path.join(model_folder_name, Model_q.name + '_acados_ocp.json')
        # load solver from json file
        build_i = True
        generate_i = True
        if gazebo_sim:
            build_i=False
            generate_i=False
        self.OCP_q_solver = AcadosOcpSolver(OCP_q,generate=generate_i,build=build_i,json_file=json_file_i)
    ...
    def MPCsolverQuadrotor_ros2_acados(self, xi_fb, xqi_traj, xl_traj, ul_traj, Ref_xi, Ref_ui, Para_i, index):
        """
        This function solves the optimal control problem (ocp) using ACADOS
        """
        # set the solver parameters at 0->N-1 nodes
        for k in range(self.N):
            ...
            uli    = np.reshape(ul_traj[k],1)
            indx   = np.reshape(index,1)
            self.OCP_q_solver.set(k,'p',np.concatenate((ref_xi,ref_ui,Para_i,xqj,xli,uli,indx)))
        ...
        for j in range(self.nq-1):
            ...
            xqjN[2*j:2*(j+1)]=xj
        xlN    = xl_traj[self.N*self.n_xl:(self.N+1)*self.n_xl]
        self.OCP_q_solver.set(self.N,'p',np.concatenate((ref_xN,ref_ui,Para_i,xqjN,xlN,uli,indx)))

        # set the initial condition to be aligned with the current feedback
        self.OCP_q_solver.set(0,'lbx',np.array(xi_fb))
        self.OCP_q_solver.set(0,'ubx',np.array(xi_fb))

        # self.OCP_q_solver.set(0,'x',np.array(xi_fb))

        NO_SOLUTION_FLAG = False

        # solve ocp
        status = self.OCP_q_solver.solve()
        ...
        controli_traj_opt = np.zeros((self.N,self.n_ui))
        for k in range(self.N):
            statei_traj_opt[k:k+1,:] = np.reshape(self.OCP_q_solver.get(k,'x'),(1,self.n_xi))
            controli_traj_opt[k:k+1,:]=np.reshape(self.OCP_q_solver.get(k,'u'),(1,self.n_ui))
        statei_traj_opt[self.N:self.N+1,:] = np.reshape(self.OCP_q_solver.get(self.N,'x'),(1,self.n_xi))
    ...
    def MPCsolverPayloadInit_acados(self,gazebo_sim=False):
        ...
        ##-------set the code generation--------##
        # compile acados ocp
        model_folder_name = f'c_generated_code_l'
        model_folder_path = os.path.join(os.getcwd(), model_folder_name)
        os.makedirs(model_folder_path, exist_ok=True)
        ocpl.code_export_directory = model_folder_path
        # compile acados ocp
        json_file_l = os.path.join(model_folder_name, modell.name + '_acados_ocp.json')
        # load solver from json file
        build_l = True
        generate_l = True
        if gazebo_sim:
